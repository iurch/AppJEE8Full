...
public class LibraryServiceContainerTest {
    ...
    //                 .withFileFromFile("target/app.war", new File(basePath(), "target/app.war")))
    //                 .waitingFor(Wait.forHttp("/app/api/application.wadl"))
    ...
    //     // 2.- Setup Webtarget again container URI
    //     String uri = String.format("http://%s:%s/app/api", container.getContainerIpAddress(),
    ...
    // // List<Book> books =
    // // api.path("/books").request().accept(MediaType.APPLICATION_JSON).get(bookList());
    // // assertThat(books, hasSize(4));
    // // }

    // // @Test
    // // public void getHitchhikersGuideToTheGalaxy() {
    // // Book book = api.path("/books/{isbn}").resolveTemplate("isbn",
    // // "0345391802").request()
    // // .accept(MediaType.APPLICATION_JSON).get(Book.class);

    // // assertThat(book, is(notNullValue()));
    // // assertThat(book.getIsbn(), is(equalTo("0345391802")));
    // // assertThat(book.getTitle(), is(equalTo("The Hitchhiker's Guide to the
    // // Galaxy")));
