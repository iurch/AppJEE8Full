...
public class BookResource {
	...
	public Response createBook(Book book) {
		...
		// if(bookshelf.exist(book.getIsbn())) return
		// Response.status(Response.Status.CONFLICT).build();

		bookshelf.create(book);
		URI location = UriBuilder.fromResource(Bookshelf.class).path("/{isbn}").resolveTemplate("isbn", book.getIsbn())
				.build();
	...
	public Response update(@PathParam("isbn") String isbn, Book book) {
		...
		// throw new WebApplicationException("ISBN must match patch paramenter",
